...
with st.spinner("Loading market data..."):
    ...
    fin, cf, news_items, holders = load_fundamentals(ticker)
...
if tgt_mean:
    ...
    for col,cls,lbl,val,sub in[
        ...
        with col: st.markdown(mc(cls,lbl,val,sub),unsafe_allow_html=True)
    st.markdown(f'''<div style="margin-top:.8rem;padding:.8rem 1.2rem;background:{rec_bg};border-radius:12px;border:1px solid {rec_col}30;display:inline-flex;align-items:center;gap:.8rem"><span style="color:{rec_col};font-size:1.1rem;font-weight:700">{rec_label}</span><span style="color:#475569;font-size:.8rem">Analyst consensus · {n_analyst} covering</span></div>''',unsafe_allow_html=True)
else:
...
if pe_series is not None and not pe_series.empty:
    cutoff   = hist_view.index[0] if not hist_view.empty else pe_series.index[0]
